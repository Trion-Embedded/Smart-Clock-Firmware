...
void button_update(button_t *btn)
{
  ...
  case IDLE:
    ...
    // attente du delay de rebond
      btn->state = DEBOUNCE_WAIT;
    }

    break;
  case DEBOUNCE_WAIT:  // attente de la fin du délai de rebond
    // Increase debaunce timer
    btn->debounce_timer++; // mesurer la durée de l'attente 

    // Check for debounce limit
    if (btn->debounce_timer > btn->debounce_ms) // fin de l'attente 
    {
      if (btn->val == btn->last_val) // si état stable à 0
      {
        btn->long_press_timer = 0; // là pour detecter un appuies long

        btn->state = CLICK_WAIT;
      }
      else // si l'état à changer on repasse en IDLE ???
      {
        // Update last value
        btn->last_val = btn->val;
        btn->state = IDLE;
    ...
    break;
  case CLICK_WAIT: // ici on attend un appuie long ou double clic
    // Increase long press timer
    btn->long_press_timer++; //  mesure la durée de l'appui long
    ...
    if (btn->val != btn->last_val)
    {
      ...
      btn->double_click_timer = 0; // RAZ du compteur 

      btn->state = DOUBLE_CLICK_WAIT;
    }

    // Check for debounce limit
    if (btn->long_press_timer > btn->long_press_ms) // cas d'un long press
    {
      btn->mode = LONG_PRESSED;
      btn->long_press_fun();

      btn->state = RELEASE_WAIT; // repasse à release wait
    }
  ...
  case DOUBLE_CLICK_WAIT:
    // Increase long press timer
    // !!!!!! FAUTE c'est double click timer !!!!!!!!!!!!!!!!!!!!!!!!
    ...
    // Check for debounce limit
    if (btn->double_click_timer > btn->double_click_ms) // temps limite pour detecter un double clic
    {
      btn->mode = CLICKED;
      btn->click_fun();
